...
- `agents/`: multi-agent pipeline system for ebook generation (9 specialized agents).
  - `agent_*.py`: Individual agent implementations (A through I).
  - `pipeline_context.py`: **NEW** - Shared context manager with caching.
  - `pipeline_orchestrator.py`: **NEW** - Concurrent execution manager.
  - `llm_client.py`: **NEW** - Unified LLM interface (Gemini + OpenRouter).
...
- Root scripts: `main.py`, `generate_manifest.py`, `run_optimized_pipeline.py` (**NEW**), `verify_*.py`.
